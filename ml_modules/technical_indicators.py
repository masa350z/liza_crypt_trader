...
* calc_sma: 単純移動平均の計算
* calc_bollinger_bands: ボリンジャーバンドの計算
* calc_macd: MACDとシグナルラインの計算
* calc_rsi: RSI(相対力指数)の計算
"""
...
def calc_sma(prices, window=20):
    ...
    出力の長さを入力価格データと同じ長さに揃え、
    最初の(window-1)要素は0.0でパディングする。

    Args:
        prices (np.ndarray): 価格データ shape = (N,)
        window (int): 移動平均の期間

    Returns:
        np.ndarray: SMA値 shape = (N,), 価格と同じスケール
    """
    N = len(prices)
